...
def affine_batch_relu_forward(x, w, b, gamma, beta, bn_params):
    ...
    - bn_params: Batch normalization parametes 

    Returns a tuple of:
    - out: Output from the ReLU
    - cache: Object to give to the backward pass
    """
    a1, fc_cache_affine = affine_forward(x, w, b)
    a2, batchnorm_cache = batchnorm_forward(a1, gamma, beta, bn_params)
    out, relu_cache = relu_forward(a2)
    cache = (fc_cache_affine, batchnorm_cache, relu_cache)
    return out, cache
...
def affine_batch_relu_backward(dout, cache):
    ...
    """
    fc_cache_affine, batchnorm_cache, relu_cache = cache
    da1 = relu_backward(dout, relu_cache)
    da2, dgamma, dbeta = batchnorm_backward_alt(da1, batchnorm_cache)
    dx, dw, db = affine_backward(da2, fc_cache_affine)
